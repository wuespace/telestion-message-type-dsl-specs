...
function print(ast, _options) {
  ...
  return (
    ...
    "<html>\n" +
    "<head>" +
    printHead(ast, options) +
    "</head>\n" +
    "<body>" +
    printBody(ast, options) +
    "</body>\n" +
    "</html>\n"
  );
}
...
function printHead(ast, options) {
  return (
    ...
    "<title>" +
    (ast.title ? ast.title.value : "Spec") +
    "</title>\n" +
    "<style>" +
    readPrismCSS() +
    "</style>\n" +
    ...
    "</style>\n" +
    "<script>" +
    readStatic("sidebar.js") +
    "</script>\n" +
    "<script>" +
    readStatic("highlightName.js") +
    "</script>\n" +
    "<script>" +
    readStatic("linkSelections.js") +
    "</script>\n" +
...
function printBody(ast, options) {
  return (
    "<article" +
    dataSourceBaseAttr(ast, options) +
    ">\n" +
    "<header>\n" +
    printTitle(ast) +
...
function printTitle(ast) {
  return !ast.title ? "" : "<h1>" + escape(ast.title.value) + "</h1>\n";
}
      ...
      if (node.type === "Section") {
        ...
        if (secID) {
          ...
          if (!nextIsLetter && IS_LETTER_RX.test(sectionIDPart)) {
            throw new Error(
              "Cannot change to numbered section " +
                nextIDPart +
                " after lettered section " +
                sectionIDPart
            );
          } else if (!nextIsLetter && nextIDPart <= sectionIDPart) {
            throw new Error(
              "Cannot change to section number " +
                nextIDPart +
                " which would be earlier than " +
                nextSecID(sectionIDPart)
            );
          }
...
function nextSecID(id) {
  ...
  const numB26 =
    "1" + letters.map((ch) => (ch.charCodeAt(0) - 65).toString(26)).join("");
  const nextDigitsB26 = (parseInt(numB26, 26) + 1).toString(26).split("");
  return nextDigitsB26
    .map((digit, index) => {
    ...
    leave: function (node) {
      if (node.type === "Section") {
        ...
        return (
          "<li>" +
          '<a href="' +
          encodeURI(options.biblio[node.id]) +
          '">' +
          '<span class="spec-secid">' +
          node.secid +
          "</span>" +
          escape(node.header) +
          "</a>" +
  ...
  const items = visit(sections, {
    leave: function (node) {
      if (node.type === "Section") {
        ...
        return (
          '<li id="' +
          escapeAttr("_sidebar_" + node.secid) +
          '">' +
          '<a href="' +
          encodeURI(options.biblio[node.id]) +
          '">' +
          ...
          "</a>" +
          (subSections &&
            '\n<input hidden class="toggle" type="checkbox" id="' +
              escapeAttr("_toggle_" + node.secid) +
              '" />' +
              '<label for="' +
              escapeAttr("_toggle_" + node.secid) +
              '"></label>\n' +
...
function printIntro(doc, options) {
  ...
  return intro.length === 0
    ? ""
    : '<section id="intro">\n' + printAll(intro, options) + "</section>\n";
}
        ...
        switch (node.type) {
          ...
          case "Header":
            ...
            return (
              ...
              '<a href="' +
              encodeURI(options.biblio[parent.id]) +
              '">' +
              escape(parent.secid) +
              "</a>" +
              "</span>" +
          ...
          case "Code":
            return (
              ...
              (node.counter
                ? ' class="spec-counter-example"'
                : node.example
                ? ' class="spec-example"'
                : "") +
              (node.lang
                ? ' data-language="' + escapeAttr(node.lang) + '"'
                : "") +
              dataSourceAttr(node, options) +
              ">" +
              (node.example
          ...
          case "Link": {
            return (
              '<a href="' +
              encodeURI(resolveLinkUrl(node.url, options)) +
              '"' +
          ...
          case "TableHeader":
            return (
              ...
              join(
                node.cells.map(
                  (cell, i) =>
                    "<th" + colAlign(node, i) + ">" + join(cell) + "</th>\n"
                )
              ) +
              "</tr>\n"
            );
          ...
          case "TableRow":
            return (
              ...
              join(
                node.cells.map(
                  (cell, i) =>
                    "<td" + colAlign(node, i) + ">" + join(cell) + "</td>\n"
                )
              ) +
              "</tr>\n"
            );
              ...
              join(
                node.rows.map(
                  (row) =>
                    "<tr>\n" +
                    join(
                      row.map((def) => '<td class="spec-rhs">' + def + "</td>")
                    ) +
                    "</tr>\n"
                )
              ) +
          ...
          case "NonTerminal":
            return (
              ...
              (node.params
                ? '<span class="spec-params">' + join(node.params) + "</span>"
                : "") +
              "</span>"
            );
          ...
          case "Quantified": {
            const quantifiers =
              (node.isList
                ? '<span class="spec-quantifier list">list</span>'
                : "") +
              (node.isOptional
                ? '<span class="spec-quantifier optional">opt</span>'
                : "");
            return (
              '<span class="spec-quantified">' +
              node.token +
              '<span class="spec-quantifiers">' +
              quantifiers +
              "</span>" +
              "</span>"
            );
...
function printIndex(ast, options) {
  ...
  return (
    ...
    join(
      termIDs.map(
        (termID) => "<li>" + link(terms[termID], termID, options) + "</li>\n"
      )
    ) +
    "</ol>\n" +
    "</section>\n"
...
function readPrismCSS() {
  return readFile(
    path.join(path.dirname(require.resolve("prismjs")), "themes/prism.css")
  )
    .replace(/\n/g, " ")
    .replace(/ {2,}/g, " ")
    .trim();
